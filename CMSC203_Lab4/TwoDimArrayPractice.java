 ...
 * Instructor: Dr. Aygun
 * Lab #4
 * Description: Work on Multi-demensional Arrays
 * Due: 10/14/2024
...
public class TwoDimArrayPractice extends JFrame
{
   ...
   */
   public void setValues( int value, int row )
   {
    ...
    // Write your code here:
	   for (int col = 0; col < intArray[row].length; col++) {
		// Set each element in the row to the given value
	    ...
	    for (int row = 0; row < intArray.length; row++) {
	        for (int col = 0; col < intArray[row].length; col++) {
	            if (intArray[row][col] == value) {
	            	// Increment the count if the value matches
	            	count++; 
	            }
	                // Animate the current comparison
